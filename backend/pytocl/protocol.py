...
class Client:
    ...
    def run(self):
        ...
        if self.state is State.STOPPED:
            ...
            try:
                _logger.info(
                    "Registering driver client with server {}.".format(self.hostaddr)
                )
    ...
    def stop(self):
        ...
        if self.state is State.RUNNING:
            ...
            _logger.info(f"Exporting csv files to '{self._output_path}'.")

            telemetry_samples_path = os.path.join(
            ...
            lap_summary_df = summarize_laps(telemetry_samples_df)
            lap_summary_df.to_csv(os.path.join(self._output_path, "laps.csv"))

            events_df = detect_events(telemetry_samples_df, session_id="session-1")
            events_df.to_csv(os.path.join(self._output_path, "events.csv"))
    ...
    def _process_server_msg(self):
        try:
            ...
            if not buffer:
                ...
                # these values may turns up negative at the start of the race
                cur_lap_time = abs(carstate.current_lap_time)
                last_lap_time = abs(carstate.last_lap_time)

                # check if a lap has been completed
                if cur_lap_time < self._last_cur_lap_time - 0.5:
                    # print(f"{cur_lap_time=} {last_lap_time=}")
                ...
                # compute current timestamp from the simulator
                timestamp_now = cur_lap_time + self._last_laps_accumulated_time
                self._last_time_diff = timestamp_now - self._timestamp_now

                # sample every 0.1s
                if self._last_time_diff >= self.TIME_PERIOD:
                    self._timestamp_now = timestamp_now
                    self._last_time_diff = 0.0

                    # print("\033[2J\033[H")
                    # print(f"{current_lap=} {last_lap_time=}")
                    ...
                    self._samples.append(next_sample)
...
class Serializer:
    ...
    def decode(buffer):
        ...
        while len(s) > pos:
            ...
            if end < 0:
                _logger.warning(
                    "Opening brace at position {} not matched in "
            ...
            if len(items) < 2:
                _logger.warning(
                    "Buffer {!r} not holding proper key value pair.".format(buffer)
                )
            else:
                key = items[0]
                if len(items) == 2:
